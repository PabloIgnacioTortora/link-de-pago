              ...
              <tbody className="divide-y divide-gray-100">
                {transactions.map((tx) => (
                  <tr key={tx._id} className="hover:bg-gray-50">
                    <td className="px-5 py-3 text-gray-700 max-w-[150px] truncate">{tx.paymentLinkId?.title ?? '—'}</td>
                    <td className="px-5 py-3 text-gray-500">{tx.payerEmail ?? tx.payerName ?? '—'}</td>
                    <td className="px-5 py-3 text-gray-500 capitalize">{tx.paymentMethod?.replace('_', ' ') ?? '—'}</td>
                    ...
                    <td className="px-5 py-3 text-gray-400 text-xs">{new Date(tx.createdAt).toLocaleDateString('es-AR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
